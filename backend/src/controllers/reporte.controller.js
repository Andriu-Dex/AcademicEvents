...
const { generarReporteEventoPDF, generarReporteMensualPDF } = require("../utils/reporte.utils");
...
async function descargarReporteMensualPDF(req, res) {
    ...
    const filePath = path.join(reportesDir, `Reporte Mensual de ${nombreMes} del ${anio}.pdf`);
    ...
    res.setHeader('Content-Disposition', `inline; filename="Reporte Mensual de ${mes} del ${anio}"`);
    res.send(pdfBuffer);
    /*res.json({
...
async function obtenerDatosReportePorMes(anio, mes) {
    ...
    const fechaInicio = new Date(`${anio}-${mes.toString().padStart(2, "0")}-01T00:00:00.000Z`);
